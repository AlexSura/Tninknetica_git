...
class Interface
  ...
  def initialize
    ...
    def new_route
      puts ' укажите дейсвие 1 - создать маршрут, 2 - добавить станцию к маршруту'
      ...
      when 1
        if Station.all.count >= 2
          ...
          puts " маршрут #{route[-1].first_station.name} - #{route[-1].last_station.name} создан"
        else
      ...
      when 3
        if self.route.empty?
          ...
          self.route.each_with_index do |route, index|
            puts " #{index} : #{route.stations.first.name} - #{route.stations.last.name} "
          end
          selected_route = gets.chomp.to_i
    ...
    def give_route
      ...
      puts " поезду #{trains[train].number} присвоен маршрут #{route[route_num].stations.first.name};
     - #{route[route_num].stations.last.name}"
    end
    ...
    def wagon_list
      ...
      if trains[number].instance_of?(CargoTrain)
        trains[number].format_list_wagon do |wagon|
          puts "вагон #{wagon.number} #{wagon.class.name}  #{wagon.free_volume_w}"
        end
      else
        trains[number].format_list_wagon do |wagon|
          puts "вагон #{wagon.number} #{wagon.class.name} свободные места #{wagon.free_places}"
        end
      end
    end
...
class IneterfaceMenu < Interface
  def initialize
    super
    puts '0 - выход'
